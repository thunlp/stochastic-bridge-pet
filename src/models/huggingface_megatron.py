...
class MegatronBertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        # Delta tuning
        self.apply_lora = config.apply_lora if hasattr(config, 'apply_lora') else False
        if self.apply_lora:
            from .delta_tuning import LoRA
            self.lora_q = LoRA(config.hidden_size, config.lora_r, config.lora_r)
            self.lora_v = LoRA(config.hidden_size, config.lora_r, config.lora_r)
    ...
    def forward(
        ...
        if self.apply_lora:
            mixed_query_layer = mixed_query_layer + self.lora_q(hidden_states)
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            if self.apply_lora:
                value_layer = self.transpose_for_scores(self.lora_v(hidden_states) + self.value(hidden_states))
            else:
...
class MegatronBertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        # Delta tuning
        self.apply_adapter = config.apply_adapter if hasattr(config, 'apply_adapter') else False
        if self.apply_adapter:
            from .delta_tuning import Adapter
            self.adapter_o = Adapter(config.hidden_size, config.adapter_r)

    def forward(self, hidden_states, residual):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        if self.apply_adapter:
            hidden_states = self.adapter_o(hidden_states)
...
class MegatronBertOutput(nn.Module):
    def __init__(self, config):
        ...
        if self.apply_adapter:
            from .delta_tuning import Adapter
            self.adapter_ff = Adapter(config.hidden_size, config.adapter_r)

    def forward(self, hidden_states, input_tensor):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        if self.apply_adapter:
            hidden_states = self.adapter_ff(hidden_states)
...
class MegatronBertForMaskedLM(MegatronBertPreTrainedModel):
    ...
    def __init__(self, config, apply_lora=False, apply_adapter=False, lora_r=8, adapter_r=8):
        super().__init__(config)
        ...
        config.apply_adapter = apply_adapter
        config.adapter_r = adapter_r
        self.bert = MegatronBertModel(config, add_pooling_layer=False)
        self.cls = MegatronBertOnlyMLMHead(config)
